...
//	beta = resCapacity /(meanDischarge * 3600 * 24 * 365/1e9);
	...
	resRelease = discharge > meanDischarge ?    							//if (Q>Qbar) {
					wetSeasonPct * discharge  :								//		resRelease = wetSeasonPct(0.16) * discharge
	 				drySeasonPct * discharge + (meanDischarge - discharge);// else
																			//		resRelease = drySeasonPct(0.6) * discharge + (meanDischarge - discharge)
