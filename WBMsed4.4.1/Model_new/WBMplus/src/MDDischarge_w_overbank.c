...
static void _MDDischargeBF (int itemID) {
	...
	//else if (Q_n == 5) Qbf  = MFVarGetFloat (_MDInBankfullQ5ID, itemID, 0.0);
	...
	//	if (QfromFP > 0) percentDiff = QfromFP/discharge;// calculate the percent difference for sediment adjustment
	...
	switch (optID) {
		...
		case MDcalculate:
			if (((_MDOutDischargeID     = MFVarGetID (MDVarDischarge,     "m3/s", MFRoute,  MFState, MFBoundary)) == CMfailed) ||
				...
				((_MDOutPCQdifferenceID = MFVarGetID (MDVarPCQdifference, " "	,MFOutput, MFState, MFInitial)) == CMfailed) ||
