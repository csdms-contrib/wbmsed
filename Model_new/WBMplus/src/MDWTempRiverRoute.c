...
//     thermal_wdl	   = MFVarGetFloat (_MDInThermalWdlID, 	   itemID, 0.0)* 1000000 / 365 / 86400;	//RJS 072011	//commented out 013112
     ...
     if(Q <= 0.0)  Q = 0.0001;							//RJS 120409
     if(Q_incoming <= 0.0) Q_incoming = 0.0001;			//RJS 120409
     ...
     if(resCapacity > 0.0){
		 ...
		 if(Q_WTemp_new > 40) Q_WTemp_new = RO_WTemp; 								//Sagy Cohen  11/05/2019
 ...
 //    }								 = %f							//RJS commented out 071511
	 ...
	 Q_WTemp_new = MDMaximum(0, (((Q_WTemp - Tequil) * exp((-HeatLoss_slope * MFModelGetLength(itemID)) / (999.73 * 4.1922 * (Q * 86400.0 / channelWidth)))) + Tequil));
 		...
 		if(Q_WTemp_new > 40) Q_WTemp_new = Tequil; 								//Sagy Cohen  11/05/2019
...
if(Q_WTemp_new > 40) Q_WTemp_new = equil2; 								//Sagy Cohen  11/05/2019

MFVarSetFloat(_MDEquil_Temp, itemID, equil2);
//printf("_MDEquil_Temp = %f, Qequil2 = %f, Q_incoming = %f\n", _MDEquil_Temp, equil2 ,Q_incoming);
...
//if( (equil2 > 20) && (Q > 0.0) && (MFModelGetLength(itemID) > 0)){
	...
	printf("RivTemp = %f, Q_WTemp_new = %f, Q_incoming = %f\n", RivTemp, Q_WTemp_new,Q_incoming);
}
...
int MDWTempRiverRouteDef () {
   ...
   if (optID==1) {
	...
	//input
	if (((_MDInDischargeID            = MDDischargeBFDef   ()) == CMfailed) || // changed by Sagy Cohen 11/5/19
