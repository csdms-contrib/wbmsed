...
/*//Printing out values to check*/
	...
	/*//printf("after Sum, y:%d\n",y);*/
		...
		/*StdDevOld = pow((sum_X_Xbar_2/((y-1)-1)),0.5);*/
	
		/*SkewOld = ((y-1)*sum_X_Xbar_3)/(((y-1)-1)*((y-1)-2)*(pow(StdDev,3)));*/
...
int MDBankfullQcalcDef() {
	...
	if ( 
       // input
       // set as initial but don't update, always use last values to calc var,
       // skew of distribution
    ((_MDInLogQMaxM2ID = MFVarGetID(MDVarLogQMaxM2, "", MFInput, MFState, MFInitial)) == CMfailed) ||
    ((_MDInLogQMaxM3ID = MFVarGetID(MDVarLogQMaxM3, "", MFInput, MFState, MFInitial)) == CMfailed) ||
    ((_MDInYearCountID = MFVarGetID(MDVarYearCount, "", MFInput, MFState, MFInitial)) == CMfailed) ||
       // output
